    ...
    for epoch in range(start_epoch+1, start_epoch+epochs+1):
        ...
        if save_checkpoint:
            ...
            torch.save(net.state_dict(), str(dir_checkpoint /
                       'p2t_checkpoint_epoch{}.pth'.format(epoch)))
            logging.info(f'Checkpoint {epoch} saved!')


    logging.info(f'final best iou = {best_iou} in epoch {best_iou_epoch} best miou = {best_miou}')
    if best_iou_epoch != 0:
        net.load_state_dict(torch.load(str(dir_checkpoint / 'p2t_checkpoint_epoch{}.pth'.format(best_iou_epoch)), map_location=device))
