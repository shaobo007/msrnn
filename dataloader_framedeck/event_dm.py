...
class EventDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        return torch.utils.data.DataLoader(self.train_dataset, batch_size=self.batch_size,
                                           ...
                                           collate_fn=self.collate_fn if self.frame_deck is True else self.collate_fn_1,
    ...
    def load_processed_file(self, f_path: str):
        ...
        '''
        voxel_image = data['voxel_image'][:, :, :, 8:328 ]
        mask = data['mask'][:, :, 8:328]
        class_id = data['class']
        gray = data['gray'].unsqueeze(1)[:, :, :, 8:328]
        '''
        voxel_image = data['voxel_image']
        mask = data['mask']
        class_id = data['class']
        gray = data['gray'].unsqueeze(1)
        #return voxel_image, mask, depth
        return voxel_image, mask, gray, class_id
    ...
    def collate_fn(batch):
        ...
        grays = [data[2] for data in batch]
        grays = torch.cat(grays,0)
        class_id = [data[3] for data in batch]
        return [images, masks, grays, class_id]
    
    @staticmethod
    def collate_fn_1(batch):
        images = [data[0][0].unsqueeze(0) for data in batch]
        images = torch.cat(images,0)
        masks = [data[1][0].unsqueeze(0) for data in batch]
        masks = torch.cat(masks,0)
        grays = [data[2][0].unsqueeze(0) for data in batch]
        grays = torch.cat(grays,0)
        class_id = [data[3] for data in batch]
        return [images, masks, grays, class_id]
