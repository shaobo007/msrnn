...
class Evimo(EventDataModule):
    ...
    def __init__(self, batch_size: int = 64, shuffle: bool = True, num_workers: int = 8,
        ...
        super(Evimo, self).__init__(img_shape=(260, 346), batch_size=batch_size, shuffle=shuffle,
    ...
    def read_annotations(self, raw_file: str, interval: int) -> List[np.ndarray]:
        MaskList = []
        ...
        for i in range(len(meta['frames'])):
            ...
            cv2.waitKey()
            MaskList.append(binary_mask)
        #print("MaskList len: ", len(MaskList))
        return MaskList
    ...
    def load(raw_file: str, interval: int):
        ...
        image_path = path.replace("/home/shaobo/evimo", "/home/shaobo/evimo/txt").replace("npz", "txt")
        ...
        gt_interval = (gt_ts[-1] - gt_ts[0]) / (len(gt_ts) - 1)
        voxel_3_channel = VoxelGrid((1, 260, 346), normalize=True)
        ...
        for i, ts in enumerate(gt_ts):
            ...
            if i % interval == 0:
                ...
                acc_image = accumulate_image(sl.T)
                AccumulateImageList.append(acc_image)
                ...
                DataList_3_channel.append(voxel_image_3_channel)
                ...
                grayImageList.append(gray_image)
                mask = undistort_img(mask_gt[i], K, D)
                col_mask = mask_to_color(mask)
                #col_mask, bounding_boxes = mask_to_color_anchor(mask)
                #Bounding_boxList.append(bounding_boxes)
                '''
                for bbox in bounding_boxes:
                    ...
                    rect_image = cv2.rectangle(acc_image, (x_min, y_min), (x_max, y_max), color=125)
                cv2.imwrite(os.path.join('/home/shaobo/temp', 'rect_image_' + str(i) + '.png'), rect_image)
                '''
                gray_mask = cv2.cvtColor(col_mask, cv2.COLOR_BGR2GRAY)
                ...
                MaskList.append(binary_mask)
                depth = undistort_img(depth_gt[i], K, D)
                #print(depth)
                #cv2.imwrite(os.path.join('/home/shaobo/temp', 'depth' + str(i) + '.png'), depth)
                #cv2.imwrite(os.path.join('/home/shaobo/temp', 'mask' + str(i) + '.png'),binary_mask)
                DepthList.append(depth)
        #print("DataList len: ", len(DataList))
        return DataList_3_channel, AccumulateImageList, MaskList, DepthList, grayImageList
    ...
    def processing(rf: str, pf: str, interval: int, resize: bool, frame_deck: bool, n_frame: int, load_func: Callable,
        ...
        data_list, image_list, mask_list, depth_list, gray_list = load_func(rf, interval)
        #assert len(data_list) == len(label_dict), "error: data_list and label_dict have different lengths"
        if frame_deck is True:
            for i, data in enumerate(data_list):
                if i % interval == 0 and i + n_frame < len(data_list):
                    obj_dict = dict()
                    '''
                    ...
                    depth_deck = [torch.from_numpy(depth_list[i+j]).unsqueeze(0) for j in range(n_frame)]
                    images = torch.cat(image_deck, 0)
                    masks = torch.cat(mask_deck, 0)
                    depths = torch.cat(depth_deck, 0)
                    flag = 1
            ...
            for i, data in enumerate(data_list):
                    ...
                    depth = cv2.normalize(depth, None, 0, 1, cv2.NORM_MINMAX, cv2.CV_32F)
                    if resize is True:
                        data = data.permute(1,2,0).numpy()
                        data, mask, depth = pad_resize_image(data, mask, depth, 256)
    ...
    def processed_files(self, mode: str) -> List[str]:
        processed_dir = os.path.join(self.root, "processed")
        return glob.glob(os.path.join(processed_dir, mode, "*", "npz", "*"))
