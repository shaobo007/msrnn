...
class recurrentUNet(nn.Module):
    def __init__(self, n_channels, n_classes, base_num_channels=32, num_encoders=4, bilinear=False, with_ca=False, model_factor=1):
...
class recurrentUNet_CA_deck(nn.Module):
    def __init__(self, n_channels, n_classes, num_frame=4, base_num_channels=32, num_encoders=4, bilinear=False, with_ca=False, recurrent_block_type='convlstm'):
        super(recurrentUNet_CA_deck, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.num_frame = num_frame
        self.bilinear = bilinear
        ...
        self.encoders = nn.ModuleList()
        for input_size, output_size in zip(self.encoder_input_sizes, self.encoder_output_sizes):
            self.encoders.append(recurrentDown_CA(input_size, output_size, recurrent_block_type=self.recurrent_block_type, with_ca= self.with_ca))
...
class recurrentUNet_CA_no_sample(nn.Module):
    def __init__(self, n_channels, n_classes, num_frame=4, base_num_channels=32, num_encoders=4, bilinear=False, with_ca=False, recurrent_block_type='convlstm'):
        super(recurrentUNet_CA_no_sample, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        self.num_frame = num_frame
        self.bilinear = bilinear
        ...
        self.encoder_input_sizes = []
        for i in range(self.num_encoders):
            self.encoder_input_sizes.append(self.base_num_channels)

        self.encoder_output_sizes = [
            self.base_num_channels for i in range(self.num_encoders)]
        self.inc = DoubleConv(n_channels, self.base_num_channels)
        self.encoders = nn.ModuleList()
        for input_size, output_size in zip(self.encoder_input_sizes, self.encoder_output_sizes):
            self.encoders.append(recurrentDown_CA_nosample(input_size, output_size, recurrent_block_type=self.recurrent_block_type, with_ca= self.with_ca))

        self.decoders = nn.ModuleList()
        for input_size, output_size in zip(reversed(self.encoder_output_sizes), reversed(self.encoder_input_sizes)):
            self.decoders.append(Up_nosample(input_size, output_size, bilinear))
        self.outc = OutConv(self.base_num_channels, self.n_classes)
        init_modules(self)
