...
class AFNet(nn.Module):
    ...
    def sub_forward(self, event, image, prev_states=None):  #x.shape (b 1) c h w
        ...
        return logits, states, corr_score, corr_image
    ...
    def forward(self, batch_events, batch_images, prev_states=None):  #batch_images.shape (b n) c h w
        ...
        for i in range(self.num_frame):
            ...
            out, prev_states, corr_score, corr_image = self.sub_forward(event, image, prev_states)  #b 1 h w
            out_list.append(out) 
        return torch.stack(out_list, dim=1), corr_score, corr_image  #b n 1 h w
