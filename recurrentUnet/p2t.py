...
class PyramidPoolingTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1, embed_dims=[64, 128, 320, 512],
        ...
        self.up1 = Up_1(embed_dims[3], embed_dims[2], bilinear=False)
        self.up2 = Up_1(embed_dims[2], embed_dims[1], bilinear=False)
        self.up3 = Up_1(embed_dims[1], embed_dims[0], bilinear=False)
        self.up4 = Up_1(embed_dims[0], 32, bilinear=True)
    ...
    def forward(self, x):
        x, out = self.forward_features_for_fpn(x)
        x = self.up1(x)
        x = self.up2(x)
        x = self.up3(x)
        x = self.up4(x)
