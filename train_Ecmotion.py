...
def train_net(net,
    ...
    # 1. Create dataset
    try:
        data_module = ecmotion(batch_size=batch_size,
    ...
    for epoch in range(start_epoch+1, start_epoch+epochs+1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for voxels, mask_true, grays in train_loader:
                ...
                if division_step > 0:
                    if global_step % division_step == 0:
                        val_score, iou, acc, iou_mean, acc_mean = evaluate_recurrentUnet_with_gray(net, val_loader, device,
                                                                                         frame_deck=frame_deck,
                                                                                         n_frame=n_frame, key_idx=0,
                                                                                         with_grays=with_grays)
        ...
        if save_checkpoint:
            ...
            torch.save(net.state_dict(), str(dir_checkpoint /
                       'R_Unet_checkpoint_epoch{}.pth'.format(epoch)))
            logging.info(f'Checkpoint {epoch} saved!')


    logging.info(f'final best iou = {best_iou} in epoch {best_iou_epoch} best miou = {best_miou}')
    if best_iou_epoch != 0:
        net.load_state_dict(torch.load(str(dir_checkpoint / 'R_Unet_checkpoint_epoch{}.pth'.format(best_iou_epoch)), map_location=device))
    ...
    Train_score, iou, acc, iou_mean, acc_mean = evaluate_recurrentUnet_with_gray(net, train_loader, device,
                                                                       frame_deck=frame_deck,
                                                                       n_frame=n_frame, key_idx=0,
                                                                       with_grays=with_grays)
    logging.info(
